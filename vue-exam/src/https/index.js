...
const https = function (config) {
  const axiosConfig = getParams(config);
  const service = axios.create({
    //baseURL: process.env.VUE_APP_REQUEST_URL, // 设置统一的请求前缀
    ...
    (config) => {
      ...
      if (custom_options.loading) {
        ...
        if (LoadingInstance._count === 1) {
          LoadingInstance._target = Loading.service(_options);
        }
      }
      // 自动携带token
 ...
 * @param {*} error
 */
const httpErrorStatusHandle = function (error) {
  // 处理被取消的请求
 ...
 * @param {*} _options
 */
const closeLoading = function (_options) {
 ...
 * @returns
 */
const getPendingKey = function (config) {
 ...
 * @return {JSON} 请求参数
 */
const getParams = function (config) {
  const { method } = config;
  const params = {
  ...
  delete config.data;
  return Object.assign(config, params[method.toLocaleUpperCase()]);
};
